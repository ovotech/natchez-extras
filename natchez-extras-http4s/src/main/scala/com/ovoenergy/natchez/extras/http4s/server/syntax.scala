...
object syntax {
  ...
  /**
   * Given an `A => F[Option[B]]` and an `A => F[B]` run the first function and if it returns F[None] then
   * fall through to the second function. This is useful for composing HttpRoutes[F] with HttpApp[F]
   */
