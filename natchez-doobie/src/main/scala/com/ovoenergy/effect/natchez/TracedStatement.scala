...
private[natchez] case class TracedStatement(
  ...
  def setClob(parameterIndex: Int, reader: Reader, length: Long): Unit = p.setClob(parameterIndex, reader, length)
  ...
  def setObject(parameterIndex: Int, x: Any, targetSqlType: Int, scaleOrLength: Int): Unit = p.setObject(parameterIndex, x, targetSqlType, scaleOrLength)
